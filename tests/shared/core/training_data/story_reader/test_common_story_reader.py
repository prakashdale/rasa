...
async def test_read_story_file_with_cycles(domain: Domain):
    graph = await training.extract_story_graph("data/test_yaml_stories/stories_with_cycle.yml", domain)
...
async def test_generate_training_data_with_cycles(domain: Domain):
...
async def test_generate_training_data_with_unused_checkpoints(
    ...
    training_trackers = await training.load_data("data/test_yaml_stories/stories_unused_checkpoints.yml", domain)
...
async def test_generate_training_data_original_and_augmented_trackers(
    ...
    training_trackers = await training.load_data(
        "data/test_yaml_stories/stories_defaultdomain.yml", domain, augmentation_factor=3,
    )
...
async def test_load_multi_file_training_data(domain: Domain):
...
async def test_load_training_data_reader_not_found_throws(
    ...
    with pytest.raises(Exception):
        await training.load_data(str(tmp_path), domain)
...
async def test_yaml_slot_different_types(domain: Domain):
